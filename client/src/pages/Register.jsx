    ...
    <div className="min-h-screen bg-hero-gradient flex items-center justify-center px-4 py-12">
      ...
      <div className="relative w-full max-w-md animate-fade-up opacity-0" style={{animationFillMode:'forwards'}}>
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="font-display font-bold text-2xl text-ink">Create your free account</h1>
          <p className="text-muted text-sm mt-1">Start analyzing your resume in under 60 seconds</p>
        </div>

        <div className="flex flex-col gap-1.5 mb-5">
          {perks.map(p => (
            <div key={p} className="flex items-center gap-2 text-sm text-ink-3">
        ...
        <div className="card shadow-card-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-ink-3 mb-1.5">Full name</label>
              <div className="relative">
                <User size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-subtle pointer-events-none" />
                <input type="text" value={name} onChange={e=>setName(e.target.value)}
                  placeholder="Alex Johnson" required className="input-field pl-10" />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" value={password} onChange={e=>setPassword(e.target.value)}
                  placeholder="Min. 8 characters" required minLength={8} className="input-field pl-10" />
              </div>
            </div>
          ...
          <p className="text-center text-xs text-subtle mt-4">By creating an account you agree to our terms of service.</p>
        </div>

        <p className="text-center text-muted text-sm mt-6">
          Already have an account?{' '}
          <Link to="/login" className="text-indigo-600 hover:text-indigo-700 font-medium transition-colors duration-150">
            Sign in
          </Link>
        </p>
      </div>
